...
# Ensure these URLs are correct for your local/CI environment
FLASK_API_URL = "http://localhost:5000/predict"
...
def test_flask_api():
    ...
    # Assert that the list is not empty (unless comments were all empty after preprocessing)
    assert len(response_data) > 0
...
def test_gemini_api():
    # You'll need a valid YouTube video ID for a successful test.
    # This ID should correspond to a video with an available transcript.
    # For testing purposes, "5_50z2nL1g0" (TED Talk) is a good choice.
    payload = {"video_id": "5_50z2nL1g0"} 
